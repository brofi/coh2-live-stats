...
def _pretty_player_table():
    ...
    table.custom_format[_COL_FACTION] = _format_faction
    table.custom_format[_COL_RANK] = partial(_format_rank, 2)
    table.custom_format[_COL_LEVEL] = partial(_format_rank, 1)
    table.custom_format[_COL_WIN_RATIO] = _format_ratio
    table.custom_format[_COL_DROP_RATIO] = _format_ratio
    table.custom_format[_COL_COUNTRY] = _format_min_max
    table.custom_format[_COL_NAME] = _format_min_max
...
def _format_ratio(f, v):
    v_str = ''
    if isinstance(v, float):
        v_str = f'{v:.0%}'
        if f == _COL_DROP_RATIO and v >= 0.1:
            v_str = colorize(Color.RED, v_str)
        elif f == _COL_WIN_RATIO:
            v_str = _format_min_max(f, (v_str, v >= 0.6, v < 0.5))
    return v_str


def _format_min_max(_, v: tuple[any, bool, bool]):
    v_str = str(v[0])
    if v[1]:
...
def _format_rank(precision, _, v: tuple[str, any, bool, bool]):
    if v[1] <= 0:
        return ''
    ...
    if isinstance(v[1], float):
        ...
        v_str = f'{v[0]}{v[1]}'
    return _format_min_max(_, (v_str, v[2], v[3]))


def _format_faction(_, v):
